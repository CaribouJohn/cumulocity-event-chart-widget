...
import { Realtime, InventoryService, EventService } from "@c8y/client";
import { DatePipe } from "@angular/common";
import moment = require("moment");
import { ThemeService } from "ng2-charts";

@Component({
    templateUrl: "./cumulocity-event-chart-widget.component.html",
    styleUrls: ["./cumulocity-event-chart-widget.component.css"],
    providers: [DatePipe, ThemeService],
})
export class CumulocityEventChartWidget implements OnInit, OnDestroy {
    @Input() config;
    ...
    setChartColors() {
        ...
        if (this.config.color !== undefined) {
            ...
            if (this.config.color === "") {
                ...
                this.barChartData[0].data.length <= this.colorsArr.length
            ) {
        ...
        if (inventory !== undefined) {
            ...
            if (devicesAll.length === 0) {
                ...
                const promises = devicesAll.map(async (device) => {
                    dataSet = await this.fetchEvents(
                        device.managedObject.id,
                        dataSet
                    );
    ...
    fetchRealtimeEvents(deviceId, dataSet) {
        ...
        const sub = this.realtimeService.subscribe(eventURL, (response) => {
            if (this.realtimeState) {
                if (response && response.data) {
                    ...
                    if (singleEvent.type === this.config.eventType) {
                        if (this.fromDate === "" || this.toDate === "") {
                            ...
                            Date.parse(singleEvent.creationTime) >
                                Date.parse(this.fromDate) &&
                            Date.parse(singleEvent.creationTime) <
                                Date.parse(this.toDate)
                        ) {
                            dataSet = this.updateDataset(
                                response.data.data,
                                dataSet
                            );
                            this.createChart(dataSet);
    ...
    async fetchEvents(deviceId, dataSet) {
        ...
        const promises = data.map((singleRecord) => {
            if (
                ...
                Date.parse(singleRecord.creationTime) >
                    Date.parse(this.fromDate) &&
    ...
    updateDataset(singleRecord, dataSet) {
        if (this.config.filter !== undefined && this.config.filter !== "") {
            if (this.config.groupby === "days") {
                const date = moment(singleRecord.creationTime).format(
                    "YYYY-MM-DD"
                );
            ...
            console.log("update called else");
